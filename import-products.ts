...
const fileContent = readFileSync('./attached_assets/Pasted-Aqui-est-uma-lista-de-100-bebidas-com-nome-pre-o-quantidade-e-marca-gerados-aleatoriamente-1--1739938374360.txt', 'utf-8');
...
async function importProducts() {
  try {
    const baseURL = 'http://0.0.0.0:3001'; // Atualizado para porta 3001

    // Faz login primeiro com as credenciais padrão do admin
    ...
    const batchSize = 10;
    const results: {
      success: { product: string; id: number }[];
    ...
    for (let i = 0; i < products.length; i += batchSize) {
      ...
      const result = await importResponse.json() as typeof results;
